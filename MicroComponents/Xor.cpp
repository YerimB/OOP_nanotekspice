...
nts::Xor::Xor(const std::string &name, uint nbA, uint nbB, uint nbO) :
...
nts::Tristate nts::Xor::compute(const std::size_t &pin)
{
    ...
    if (this->_pins.at(0)->getState() == nts::UNDEFINED or \
    this->_pins.at(1)->getState() == nts::UNDEFINED)
        this->_pins.at(2)->setState(nts::UNDEFINED);
    else if (this->_pins.at(0)->getState() == this->_pins.at(1)->getState())
