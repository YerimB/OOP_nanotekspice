...
nts::Or::Or(const std::string &name, uint nbA, uint nbB, uint nbO) :
...
nts::Tristate nts::Or::compute(const std::size_t &pin)
{
    ...
    if (this->_pins.at(0)->getState() == nts::TRUE or \
    this->_pins.at(1)->getState() == nts::TRUE)
